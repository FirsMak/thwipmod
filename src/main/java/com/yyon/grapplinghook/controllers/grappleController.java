...
//	public SegmentHandler segmenthandler;
	...
	public boolean pullweb = false;
	public boolean startweb = false;

	public boolean prevKeySpace = false;


	public grappleController(int arrowId, int entityId, World world, vec motion, int controllerid, GrappleCustomization custom) {
	...
	public void unattach() {
		if (grapplemod.controllers.containsValue(this)) {
			...
			if (this.controllerid != grapplemod.AIRID) {
				...
				grapplemod.proxy.createControl(grapplemod.AIRID, -1, this.entityId, this.entity.world, motion, null, this.custom);
			}
		}
	}
		...
		if (this.attached) {
			if(entity != null) {
				if (true) {
					...
					if (ClientProxyClass.key_jumpanddetach.isKeyDown() != prevKeySpace){
						if (ClientProxyClass.key_jumpanddetach.isKeyDown()){
							pullweb = true;
						}
						prevKeySpace = ClientProxyClass.key_jumpanddetach.isKeyDown();
					}
...
//					}
					...
					for (grappleArrow arrow : this.arrows) {
						arrow.prevr = ((grappleArrow) arrow).segmenthandler.getDist(vec.positionvec(arrow), vec.positionvec(entity).add(new vec(0, entity.getEyeHeight(), 0))) ;

						if (startweb){
							arrow.initLength += 1.25;
							RenderGrappleArrow.startweb = true;
						...
						BlockPos blockPos = new BlockPos(entity.posX,entity.posY-0.001, entity.posZ);
						if (entity.world.getBlockState(blockPos).getBlock() != Blocks.AIR){
							tasmHelp = 0;
							System.out.println("TASM IN BOTTTOM");
						}
						else {
							System.out.println("TASM IN AIR");
							tasmHelp += 1;
						}
						...
						if (custom.getBoolean("tasmmode") && tasmHelp > 10){
							if (arrow.initLength < arrow.prevr){
								arrow.initLength = arrow.prevr;
							}
							remaininglength = motor ? Math.max(this.custom.maxlen, arrow.initLength) - distToAnchor : arrow.initLength - distToAnchor;
						}
						else{
							remaininglength = motor ? Math.max(this.custom.maxlen, arrow.prevr) - distToAnchor : arrow.prevr - distToAnchor;
						}
						...
						if (pullweb) {
							if (!Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)){
								if (!((distToAnchor + oldspherevec.length()) > arrow.initLength)){
									arrow.initLength = distToAnchor + oldspherevec.length();
								}
							}
						}
						...
						if (custom.tasmMode && tasmHelp > 10){
							...
							double angle = spherevec.angle(motion);
							double elasticModules = custom.getDouble("sc");
							spherevec.inverse();
							double changelenweb = (arrow.prevr-arrow.initLength)/arrow.initLength;
							double s = 4 * Math.pow(10,-6);
							double acc = elasticModules*s*changelenweb/mass;
							vec accnormal = motion.mult(Math.cos(angle));
							vec motionsphere = motion.mult(Math.cos(motion.angle(spherevec)));
							double dlen = (mass*accnormal.length()*arrow.initLength)/(elasticModules*s);
							if (arrow.prevr > arrow.initLength){
								if (entity.world.getBlockState(blockPos).getBlock() != Blocks.AIR){
									if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || pullweb){
										motion.add_ip(spherevec.normalize().mult(acc/5));
									}
									else {
										motion.add_ip(spherevec.normalize().mult(acc/35));
									}
								}
								else {
									if(spherevec.normalize().mult(acc).length() > 35){
										motion.add_ip(spherevec.normalize().mult(acc).normalize().mult(15));
									}
									else {
										motion.add_ip(spherevec.normalize().mult(acc));
									}
								}
							}
						}
		        	...
		        	// Motor
					if (pullweb) {
						if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) && !custom.tasmMode){
							for (grappleArrow arrow : this.arrows){
								arrow.initLength -= 1.5;
							}
							pullweb = false;
						}
						else {
...
/*	public void applyWaterFriction() {
	...
	public boolean accRange = true;
	public void applyPlayerMovement() {
		double i = ClientProxyClass.controlId;
		if(Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) && accRange && !pullweb){
			i = i + 14;
			accRange = false;
		}
		playermovement = playermovement_unrotated.rotate_yaw((float) (this.entity.rotationYaw * (Math.PI / 180.0)));

		motion.add_ip(this.playermovement.changelen(0.015 + motion.length() * 0.01).mult(this.playermovementmult * i));//0.02 * playermovementmult));
	}

	public void addArrow(grappleArrow arrow) {
		this.arrows.add(arrow);

		arrow.initLength = ((grappleArrow) arrow).segmenthandler.getDist(vec.positionvec(arrow), vec.positionvec(entity).add(new vec(0, entity.getEyeHeight(), 0)));
